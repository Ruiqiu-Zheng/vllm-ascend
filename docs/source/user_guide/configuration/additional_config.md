...
| `enable_sfa_dcp_sharded_indexer`    | bool | `False` | Prepare rank-local LightningIndexer candidates for an exact global TopK merge in the bounded GLM-5.2 DCP16/B128/K2048 decode path, with global visible length capped at 256K. This source candidate is default-off and does not support prefill, speculative decoding, DSA-CP, KV transfer, or LI C8. |
