...
def test_compose_indexer_cache_metadata_uses_replicated_dcp_views():
    local_slots = torch.tensor([11, 12, 13], dtype=torch.int32)
    replicated_slots = torch.tensor([101, 102, 103], dtype=torch.int32)
    # #15809's PCP+DCP builder additionally retains the slot mapping in PCP
    # gather order. The PCP cache-write helper gathers K only; it consumes this
    # precomputed slot order directly instead of gathering/reordering slots.
